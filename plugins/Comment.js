...
import { Loading } from '@/plugins/LoadingProgress';

import { siteConfig } from '@/libs/common/config';
import { isSearchEngineBot } from '@/libs/common/utils';

const WalineComponent = dynamic(
  () => {
    return import('@/plugins/WalineComponent');
  },
  { ssr: false }
);
...
const Comment = ({ post }) => {
  ...
  return (
    <div
      id="comment"
      key={post?.id}
      className="mt-5 text-gray-800 dark:text-gray-300 overflow-hidden"
    >
      <Tabs>
