...
export const MenuItemDrop = ({ link }) => {
  ...
  return <div onMouseOver={() => changeShow(true)} onMouseOut={() => changeShow(false)} >
        ...
        {!hasSubMenu &&
            <Link
                href={link?.to}
                className="font-sans menu-title pl-2 pr-4 text-gray-700 dark:text-gray-200 no-underline tracking-widest pb-1">
                {link?.icon && <i className={link?.icon}/>} {link?.name}
                {hasSubMenu && <i className='px-2 fa fa-angle-down'></i>}
            </Link>}

        {hasSubMenu && <>
            <div className='cursor-pointer font-sans menu-title pl-2 pr-4 text-gray-700 dark:text-gray-200 no-underline tracking-widest pb-1'>
                {link?.icon && <i className={link?.icon}/>} {link?.name}
