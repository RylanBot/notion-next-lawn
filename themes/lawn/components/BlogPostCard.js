...
const BlogPostCard = ({ index, post, showSummary, siteInfo }) => {
  ...
  return (
        <div className={`${siteConfig('LAWN_POST_LIST_COVER_HOVER_ENLARGE', null, CONFIG) ? ' hover:scale-110 transition-all duration-150' : ''}`} >
            <div key={post.id}
                data-aos="fade-up"
                ...
                id='blog-post-card'
                className={`group md:h-64 w-full flex justify-between md:flex-row flex-col-reverse ${siteConfig('LAWN_POST_LIST_IMG_CROSSOVER', null, CONFIG) && index % 2 === 1 ? 'md:flex-row-reverse' : ''}
                    overflow-hidden border-2 border-teal-600 dark:border-teal-500 rounded-lg bg-white dark:bg-lawn-black-gray`}>
