...
export const MenuItemCollapse = ({ link }) => {
  ...
  return <>
        <div onClick={toggleShow} className={'py-2 px-5 duration-300 text-base justify-between hover:bg-teal-700 hover:text-white hover:shadow-lg cursor-pointer font-light flex flex-nowrap items-center ' +
            (selected ? 'bg-teal-500 text-white ' : ' text-black dark:text-white ')}>
        ...
        {hasSubMenu && <Collapse isOpen={isOpen}>
            {link.subMenus.map((sLink, index) => {
              return <div key={index} className='cursor-pointer whitespace-nowrap dark:text-gray-200  w-full font-extralight dark:bg-black text-left px-5 justify-start bg-gray-100  hover:bg-teal-700 hover:text-white dark:hover:bg-gray-900 tracking-widest transition-all duration-200 border-b dark:border-gray-800 py-3 pr-6'>
