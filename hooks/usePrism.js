...
const renderCollapseCode = (expandDefault) => {
  ...
  for (const codeBlock of codeBlocks) {
    ...
    panelWrapper.className =
      'border dark:border-gray-600 rounded-md hover:border-teal-500 duration-300 transition-colors';

    const header = document.createElement('div');
    header.className = 'flex justify-between items-center px-4 py-2 cursor-pointer select-none';
    header.innerHTML = `<h3 class="text-lg font-medium">${language}</h3><svg class="transition-all duration-300 w-5 h-5 transform rotate-0" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M6.293 6.293a1 1 0 0 1 1.414 0L10 8.586l2.293-2.293a1 1 0 0 1 1.414 1.414l-3 3a1 1 0 0 1-1.414 0l-3-3a1 1 0 0 1 0-1.414z" clip-rule="evenodd"/></svg>`;

    const panel = document.createElement('div');
    panel.className = 'invisible h-0 transition-transform duration-300 border-t border-gray-300';
